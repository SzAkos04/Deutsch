---
Titel: Zeitformen (Igeidők)
tags:
  - Grammatik
Wortschatz: "[[Zeitformen]]"
---
# Zeitformen (Igeidők)

## 1. Präsens (Jelen idő)

### A) Alapszabály

A jelen időt használjuk a **most** történő eseményekre, az **általános igazságokra**, és a németben meglepő módon a **biztos jövőre** is (ha van a mondatban időhatározó, pl. _morgen_).

### B) Ragozás

Lásd: [[Konjugation (Igeragozás)]]

---

## 2. Perfekt (Összetett múlt idő)

### A) Alapszabály

A hétköznapi beszédben **90%-ban ezt használjuk,** ha a múltról beszélünk. Két elemből áll, mint egy szendvics: a segédigéből és a múlt idejű alakból.

**Szerkezet:** haben/sein (ragozva) + ... + Partizip II (mondat végén)

### B) Segédigék választása

**Mikor használunk _sein_-t?**

1. **Mozgás** (A-ból B-be): _gehen, kommen, laufen_
2. **Állapotváltozás:** _aufstehen_ (felkelni), _einschlafen_ (elaludni)
3. **Közlekedés:** _fahren_
4. **Kivételek:** _sein, werden, bleiben_

Minden más esetben (a legtöbb ige) a **haben**-t használjuk.

> [!TIP] **MÁK**-os igék _sein_-osak (**M**ozgás, **Á**llapotváltozás, **K**özlekedés)

### C) Partizip II (3. alak) típusai

1. **Gyenge:** _ge- + tő + -t_ (pl. _ge-mach-t_)
2. **Erős:** _ge- + tő (változhat) + -en_ (pl. _ge-seh-en_)
3. **-ieren végű:** nincs _ge-_, csak _-t_ a végén (pl. _studiert_)

**Példák:**
- _Ich **habe** gestern Pizza **gegessen**._ (Tegnap pizzát ettem.)
- _Wir **sind** nach Hause **gegangen**._ (Hazamentünk.)

---

## 3. Präteritum (Egyszerű múlt idő)

### A) Alapszabály

Főleg írásban (mesék, újságok) használjuk. Beszédben csak a leggyakoribb igéknél (létige, birtoklás, módbeli segédigék) váltja fel a Perfektet.

### B) Legfontosabb Präteritum alakok

| Alany         | sein (volt) | haben (volt valamije) | können (tudott) |
| ------------- | ----------- | --------------------- | --------------- |
| **ich**       | war         | hatte                 | konnte          |
| **du**        | warst       | hattest               | konntest        |
| **er/sie/es** | **war**     | **hatte**             | **konnte**      |
| **wir**       | waren       | hatten                | konnten         |
| **ihr**       | wart        | hattet                | konntet         |
| **sie/Sie**   | waren       | hatten                | konnten         |

> [!TIP] **Beszélt nyelvi szabály:** Ha azt akarod mondani, hogy "Voltam" vagy "Nekem volt", mindig a _war_ és _hatte_ alakokat használd a Perfekt helyett, mert sokkal természetesebb.

---

## 4. Futur I (Jövő idő)

### A) Alapszabály

Akkor használjuk, ha hangsúlyozni akarjuk a jövőt, vagy ha jóslatot/ígéretet teszünk.

**Szerkezet:** werden (ragozva) + Infinitiv (mondat végén)

### B) _werden_ ige ragozása

|**Alany**|**werden ragozva**|
|---|---|
|**ich**|werde|
|**du**|wirst|
|**er/sie/es**|wird|
|**wir/sie/Sie**|werden|
|**ihr**|werdet|

---

## 5. Futur II (Befejezett jövő idő)

### A) Alapszabály

A Futur II-t két esetben használjuk:
1. **Befejezettség a jövőben:** Egy olyan eseményről beszélünk, ami a jövő egy adott pontján már **le fog zárulni**. (Gyakran szerepel mellette időhatározó: _bis morgen, in zwei Jahren_).
2. **Feltételezés a múlttal kapcsolatban:** Amikor nem tudunk valamit biztosan, de feltételezzük, hogy megtörtént. (_"Biztos elfelejtette."_)

**Szerkezet:** werden (ragozva) + ... + Partizip II + haben/sein (Infinitiv)

> [!TIP] **A mondat végi sorrend:** A mondat legvégén az **Infinitiv** (_haben_ vagy _sein_) áll, közvetlenül előtte pedig a **Partizip II**. A _werden_ pedig marad a 2. helyen.

**Példák:**
- _Ich werde es bis morgen abgeschlossen haben._ (Holnapra már be fogom fejezni.)
- _Er wird angekommen sein._ (Valószínűleg már megérkezett. – _Feltételezés_)

### B) Segédigék választása

Ugyanaz a szabály érvényes, mint a **[[Zeitformen (Igeidők)#B) Segédigék választása|Perfekt]]**-nél:
- **sein:** MÁK-os igéknél (pl. _gegangen sein, eingeschlafen sein_).
- **haben:** minden más (pl. _gelernt haben, gemacht haben_).

### C) Gyakori használat a hétköznapokban (Feltételezés)

A németek nagyon szeretik a Futur II-t találgatásra használni a múltra vonatkozóan. Ha látsz egy üres sörösüveget az asztalon, mondhatod:
- _Er **wird** durstig **gewesen sein**._ (Biztosan szomjas volt.)

> [!INFO] **Tipp:** Ha a jövőbeli befejezettséget akarod kifejezni, a németek sokszor helyettesítik a sima **Perfekt**-tel, ha van benne időhatározó:
> 
> - _Bis morgen **habe** ich es **fertig gemacht**._ (Holnapra kész vagyok vele.) – Ez sokkal gyakoribb élő szóban, mint a Futur II.

